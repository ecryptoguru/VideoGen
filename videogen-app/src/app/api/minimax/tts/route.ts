...
export async function POST(req: NextRequest) {
  ...
  const endpoint = "/api/minimax/tts";
  const method = "POST";
  ...
  try {
    ...
    const sanitizedText = sanitizeInput(requestBody.text);
    
    const filterResult = filterPrompt(sanitizedText);
    ...
    const piiCheck = detectPII(sanitizedText);
    if (piiCheck.hasPII) {
      apiLogger.logRequest({
        timestamp: new Date().toISOString(),
        endpoint,
    ...
    if (sanitizedText.length > 10000) {
      ...
      recordApiUsage({
        ...
        cost_cents: 0,
        status: "validation_failed",
        error_message: "text exceeds 10,000 character limit",
      });
    ...
    const response = await retryWithBackoff(async () => {
      ...
      const res = await fetch("https://api-uw.minimax.io/v1/t2a_v2", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          model,
          text: sanitizedText,
          voice_setting: requestBody.voice_setting || { voice_id: "English_expressive_narrator" },
          audio_setting: requestBody.audio_setting || { sample_rate: 32000, bitrate: 128000, format: "mp3" },
        }),
