...
export async function POST(req: NextRequest) {
  ...
  const endpoint = "/api/minimax/music";
  const method = "POST";
  ...
  try {
    ...
    // Check for PII in prompt and lyrics
    const allText = sanitizedPrompt + " " + (requestBody.lyrics || "");
    const piiCheck = detectPII(allText);
    if (piiCheck.hasPII) {
      apiLogger.logRequest({
        timestamp: new Date().toISOString(),
        endpoint,
    ...
    const response = await retryWithBackoff(async () => {
      ...
      const res = await fetch("https://api.minimax.io/v1/music_generation", {
        ...
        body: JSON.stringify({
          ...
          is_instrumental: requestBody.is_instrumental || false,
          output_format: "hex",
        }),
    ...
    console.error("MiniMax music API error:", error);
    apiLogger.logRequest({
      timestamp: new Date().toISOString(),
      endpoint,
      method,
