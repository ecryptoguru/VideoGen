...
export async function POST(req: NextRequest) {
  ...
  const endpoint = "/api/minimax/image";
  const method = "POST";
  ...
  try {
    ...
    if (sanitizedPrompt.length > 1500) {
      apiLogger.logRequest({
        timestamp: new Date().toISOString(),
        endpoint,
        method,
        status: 400,
        duration: Date.now() - startTime,
        error: "prompt validation failed - too long",
      });
      await recordApiUsage({
        user_id: userId,
        project_id: projectId,
        ...
        cost_cents: 0,
        status: "validation_failed",
        error_message: "prompt validation failed - too long",
      });
      return NextResponse.json({ error: "prompt is required and must be under 1500 chars" }, { status: 400 });
    }
    ...
    const response = await retryWithBackoff(async () => {
      ...
      const res = await fetch("https://api.minimax.io/v1/image_generation", {
        ...
        body: JSON.stringify({
          ...
          aspect_ratio: requestBody.aspect_ratio || "1:1",
          n: requestBody.n || 1,
